(() => {
  const allLinks = Array.from(document.querySelectorAll('.gallery-masonry-lightbox-link, .gallery-link, [data-lightbox]'));
  if (!allLinks.length) return;
  // The active set excludes links hidden by a category filter, so navigation
  // stays within whatever the visitor is currently viewing.
  let links = allLinks;
  let currentIndex = 0;
  ...
  allLinks.forEach((link) => {
    link.addEventListener('click', (event) => {
      ...
      links = allLinks.filter((candidate) => candidate.offsetParent !== null);
